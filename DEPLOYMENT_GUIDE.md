...
> Tarih: 2026-05-06
> Varsayım: Domain alındı (örnek olarak `pruvio.com` kullanılacak; sen kendi domain'inle değiştir).
...
- [ ] UptimeRobot'a `/actuator/health` ve `https://pruvio.com`'u ekle
...
2. **Sender → Senders & IP → Add a Sender**:
   ...
   - From email: `noreply@pruvio.com` (henüz domain DNS hazır olmasa bile sırayla yap)
3. **SMTP & API → SMTP**:
...
3. Daha sonra (deploy bittikten sonra) **Add New Monitor**:
   - Type: HTTP(s)
   - URL: `https://api.pruvio.com/actuator/health`
   - Interval: 5 minutes
   - Alert contacts: e-posta (kendi adresin)
4. Bir tane de frontend için: `https://pruvio.com`
...
flyctl secrets set \
  ...
  MAIL_USERNAME="..." \
  MAIL_PASSWORD="..." \
  MAIL_FROM="noreply@pruvio.com" \
  CORS_ALLOWED_ORIGINS="https://pruvio.com" \
...
export const environment = {
  production: true,
  apiUrl: 'https://api.pruvio.com/api',  // ← gerçek backend URL'in
...
2. `pruvio.com` ve `www.pruvio.com` ekle
...
| `TXT` | `_dmarc` | `v=DMARC1; p=quarantine; rua=mailto:dmarc@pruvio.com` | — | DMARC |
...
| `MAIL_FROM` | Fly.io secret | `noreply@pruvio.com` |
| `MAIL_ENABLED` | Fly.io secret | `true` |
| `CORS_ALLOWED_ORIGINS` | Fly.io secret | `https://pruvio.com,https://www.pruvio.com` |
...
1. [ ] `https://pruvio.com` açılıyor, login ekranı geliyor
...
8. [ ] `https://api.pruvio.com/actuator/health` → 200 + `{"status":"UP"}`
